...
def captureImage():
    ...
    while(vidcap.isOpened()):
        ...
        if(int(vidcap.get(1)) % 30 == 0):
            cv2.imwrite(video_path + "frame%d.png"% count, image)
            count += 1
            if count == 4:
                break
    vidcap.release()
...
for kk in range(number_files):
    img_file = folder + '/frame%d.png' %kk
